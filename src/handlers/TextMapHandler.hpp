...
class TextMapHandler : public BaseMapHandler
{
	...
	private:
        ...
        int readLayers(std::ifstream& file, Map& map);
        int readBackgrounds(std::ifstream& file, Map& map);
        int readCollision(std::ifstream& file, Map& map);
};
